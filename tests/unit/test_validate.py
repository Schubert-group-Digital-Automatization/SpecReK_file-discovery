"""Unit tests for validate behavior."""
...
def test_validate_dir_exists_rejects_file_path(tmp_path: Path) -> None:
    """Verify validate dir exists rejects file path."""
...
def test_validate_csv_file_accepts_existing_nonempty_csv(tmp_path: Path) -> None:
    """Verify validate CSV file accepts existing nonempty CSV."""
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            lambda tmp: tmp / "missing.csv", FileNotFoundError, "curated_csv", id="missing"
        ),
        pytest.param(lambda tmp: tmp, IsADirectoryError, "curated_csv", id="directory"),
        pytest.param(
            lambda tmp: write_text(tmp / "registry.txt", "ID;Path\n"),
            ValueError,
            ".csv",
            id="suffix",
        ),
        pytest.param(
            lambda tmp: write_text(tmp / "empty.csv", ""), ValueError, "empty", id="empty"
        ),
    ],
)
def test_validate_csv_file_rejects_invalid_paths(
    ...
    """Verify validate CSV file rejects invalid paths."""
...
def test_validate_csv_has_required_columns_accepts_header_only_csv(tmp_path: Path) -> None:
    """Verify validate CSV has required columns accepts header only CSV."""
...
def test_validate_csv_has_required_columns_rejects_unparseable_headers(
    ...
    """Verify validate CSV has required columns rejects unparseable headers."""
...
def test_validate_csv_has_required_columns_reports_missing_columns(
    ...
    """Verify validate CSV has required columns reports missing columns."""
...
def test_validate_csv_has_required_columns_rejects_invalid_header_columns(
    ...
    """Verify validate CSV has required columns rejects invalid header columns."""
...
def test_validate_csv_has_required_columns_rejects_empty_separator(tmp_path: Path) -> None:
    """Verify validate CSV has required columns rejects empty separator."""
...
def test_validate_output_parent_exists_accepts_existing_parent(tmp_path: Path) -> None:
    """Verify validate output parent exists accepts existing parent."""
...
def test_validate_output_parent_exists_rejects_missing_parent(tmp_path: Path) -> None:
    """Verify validate output parent exists rejects missing parent."""
...
def test_validate_output_parent_exists_rejects_parent_file(tmp_path: Path) -> None:
    """Verify validate output parent exists rejects parent file."""
